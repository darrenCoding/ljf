...
compile.init = function (env) {
    var mapFile = path.join(process.cwd(), '/', ljserver.config.build.resource);
    this.content = ljserver.file.readJSON(mapFile);
    this.env = env || 'dev';
    this.buildFile = path.join(process.cwd(), '/', ljserver.config.build.create);
    if ( this.content ) {
        ljserver.log.info('Using resourceMap.json %s', mapFile)
...
compile.parse = function () {
    this.buildObj = {};
    var rules = this.content.rules;
    ljserver.util.eachOf(Object.keys(rules), function (ikey, index, go) {
        var _assets = [],
            _urlArr = [];
        if ( Array.isArray(rules[ikey]) ) {
            rules[ikey].forEach(function (asset) {
                if ( ljserver.util.isObject(asset) ){
                    ...
                    if ( asset in this.content['resource'] ) {
                        ...
                        ljserver.log.error('Uncaught ReferenceError: %s is not defined', asset);
                    }
                }
            },this)
        } else {
            if ( rules[ikey] in this.content['resource'] ) {
                ...
                ljserver.log.error('Uncaught ReferenceError: %s is not defined', rules[ikey]);
            }
        }
        
        ljserver.util.eachOfSeries(_assets, function (iurl, i, next) {
            this.comboFiles(iurl, function (combine) {
                ljserver.log.info(combine);
                _urlArr.push(combine);
                next();
            });
        }, function () {
            var _env = this.content['domain'][this.env].replace(/(http|https):\/\//,'')
            this.buildObj[_env + ikey] = _urlArr;
            go();
        }, this)
    }, function () {
        this.injectStore(function () {
...
compile.comboFiles = function (obj, cb) {
    var _root = process.cwd().replace(cjson.svndev, '') + '/' ;
    obj.asset = Array.isArray(obj.asset) ? obj.asset : [obj.asset];
    obj.asset = obj.asset.map(function (url, i) {
        return url in this.content['alias'] ? this.content['alias'][url] : url;
    },this)
    var _ext = path.extname(obj.asset[0]) || '',
        _urlPath = obj.asset.sort(function (fp) {
        return path.isAbsolute(fp) ? 1 : -1
    }).map(function (item, index) {
        var _base = path.dirname(item) + '/';
    ...
    this.judgeMd5(this.wholePath).then(function (url) {
...
compile.judgeMd5 = function (url, md5) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        combo(url, function (err, data, deps) {
            if ( err ){
                reject(err);
            } else {
...
compile.injectStore = function (cb) {
    if ( this.content.localStorage ) {
        var _basicArr = [];
        ljserver.util.eachOf(this.content.localStorage, function (val, key, go) {
            this.judgeMd5('//' + ljserver.config.build.domain + val, true).then(function(md5){
            ...
            _basicArr = _basicArr.map(function (local) {
                var _lp = local.split("&&");
